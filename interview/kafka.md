...
네트워크 성능 향상  
* Kafka는 내부적으로 파일 시스템을 사용합니다. 
* 클라이언트가 메시지를 한 건씩 자주 보내면, 브로커는 이를 계속 처리하느라 성능이 떨어질 수 있습니다. 
* Kafka는 이를 해결하기 위해 message set이라는 구조로 여러 메시지를 한 번에 묶어서 처리합니다. 
* 이렇게 하면 네트워크 왕복 요청 횟수(RTT)를 줄일 수 있어 성능이 좋아집니다.
* 메시지를 한 건씩 보내면 매번 네트워크 요청이 발생 → TCP/IP 핸드셰이크, 헤더 등 부가 비용 큼 
* 일괄 전송하면 여러 메시지를 한 번에 묶어서 전송 → 네트워크 사용량 효율 증가

디스크 쓰기 효율 증가  
* Kafka 브로커는 메시지를 디스크에 저장합니다. 
* 메시지를 하나씩 저장하면 디스크에 계속 접근해야 해서 성능이 나빠질 수 있습니다. 
* Batch를 사용하면 메시지를 덩어리로 묶어 한 번에 저장할 수 있습니다. 
* 이 덕분에 Kafka는 **순차적인 디스크 쓰기(sequential write)**를 크게 늘릴 수 있습니다.
* 즉, batch 단위로 쓰면 한 번의 디스크 write로 여러 메시지를 저장 → 디스크 쓰기 효율 증가

압축 효율 증가  
* Kafka는 네트워크가 병목이 될 경우를 대비해, Batch 단위로 압축하는 기능도 제공합니다. 
* 여러 메시지를 하나의 batch로 묶으면 비슷한 패턴이 많아 압축률이 높아지고, 전송 데이터량이 줄어듭니다. 
* 덕분에 네트워크 사용량을 줄이고 처리량을 늘릴 수 있습니다.
* 즉, 압축된 데이터를 전송/저장하므로 네트워크 대역폭 절약
